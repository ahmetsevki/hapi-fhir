...
public class JaxRsExceptionInterceptor {
	 ...
	 */
	public JaxRsResponseException convertException(
			final AbstractJaxRsProvider theServer, final Throwable theException) {
	 ...
	 * @throws IOException
	 */
	public Response convertExceptionIntoResponse(
			final JaxRsRequest theRequest, final JaxRsResponseException theException) throws IOException {
		return handleExceptionWithoutServletError(theRequest, theException);
	}

	private BaseServerResponseException preprocessException(
			final Throwable theException, final JaxRsRequest requestDetails) {
		try {
			Throwable theExceptionToConvert = theException;
			if (!(theException instanceof BaseServerResponseException)
					&& (theException.getCause() instanceof BaseServerResponseException)) {
	...
	private Response handleExceptionWithoutServletError(
			final JaxRsRequest theRequest, final BaseServerResponseException theException) throws IOException {
		try {
			return (Response) exceptionHandler.handleException(theRequest, theException);
		} catch (final ServletException e) {
			final BaseServerResponseException newException =
					preprocessException(new InternalErrorException(e), theRequest);
